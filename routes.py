...
def init_default_data():
    ...
    if GlobalPricingConfig.query.count() == 0:
        config = GlobalPricingConfig(
            minimum_billable_weight=2.0,
            base_pickup_charge=100.0,
            extra_charge_per_kg=30.0,
...
def place_order():
    ...
    if request.method == 'POST':
        try:
            ...
            insurance_value = float(request.form.get('insurance_value', 0.0))
            
            # Validate required fields
            if not all([customer_name, customer_email, customer_phone, pickup_address, 
                       delivery_address, zone_id, weight, length, width, height, 
...
@app.route('/admin/pricing-config')
@admin_required
def admin_pricing_config():
    ...
    return render_template('admin_pricing_config.html', config=config, zones=zones)
...
def save_global_config():
    ...
    try:
        ...
        config.minimum_billable_weight = float(request.form.get('minimum_billable_weight', 2.0))
        config.base_pickup_charge = float(request.form.get('base_pickup_charge', 100.0))
        config.extra_charge_per_kg = float(request.form.get('extra_charge_per_kg', 30.0))
        config.insurance_rate = float(request.form.get('insurance_rate', 0.005))
        config.cod_fee_rate = float(request.form.get('cod_fee_rate', 0.02))
        config.card_payment_fee_rate = float(request.form.get('card_payment_fee_rate', 0.015))
...
def initialize_contact_settings():
    ...
    try:
        ...
        existing_settings = ContactSettings.query.first()
        if existing_settings:
            print("Contact settings already exist, skipping initialization")
            return
        
        # Create default contact settings
        default_settings = ContactSettings(
            company_name='GotoFast Logistics',
            ...
            support_email='support@gotofast.com',
            business_hours='Monday - Friday: 9:00 AM - 6:00 PM',
            facebook_url='https://facebook.com/gotofast',
            twitter_url='https://twitter.com/gotofast',
            linkedin_url='https://linkedin.com/company/gotofast',
            instagram_url='https://instagram.com/gotofast'
        )
        ...
        print(f"Error initializing contact settings: {e}")
        db.session.rollback()
