...
class GlobalPricingConfig(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    minimum_billable_weight = db.Column(db.Float, nullable=False, default=2.0)  # kg
    base_pickup_charge = db.Column(db.Float, nullable=False, default=100.0)  # ₹
    extra_charge_per_kg = db.Column(db.Float, nullable=False, default=30.0)  # ₹/kg
    insurance_rate = db.Column(db.Float, nullable=False, default=0.005)  # 0.5%
    cod_fee_rate = db.Column(db.Float, nullable=False, default=0.02)  # 2%
    card_payment_fee_rate = db.Column(db.Float, nullable=False, default=0.015)  # 1.5%
    gst_rate = db.Column(db.Float, nullable=False, default=0.18)  # 18%
    updated_by = db.Column(db.Integer, db.ForeignKey('admins.id'))
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
